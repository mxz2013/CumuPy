...
def calc_ShiftImSig(en, ims_tmp, ikeff, ibeff ,Elda_kb, xfermi, Eplasmon,
    ...
    if rc_toc == 0: ## toc calculation 
        ...
        NewEn_min =  -6*Eplasmon #+ Elda_kb  #default calculate 6 plasmon
...
def calc_FFT0(eqp_kb, gt_list, fftsize,dtfft, enmin, newen_toc, denfft, invar_eta):
    ...
    gwlist = []
    for w in wlist:
        Area2 = s_go/(w-eqp_kb-s_freq-eta) # here eqp=ehf 
        c = np.trapz(Area2, dx = denfft)
        cwIm = 1./np.pi*c.imag
        gwlist.append(0.5/np.pi*cwIm)
...
def calc_toc(wps1,wps2,gwfermi,lda_fermi, bdrange, bdgw_min, kptrange, FFTtsize, en,enmin, enmax,
                    eqp, Elda, scgw, Eplasmon, ims, invar_den,
                    invar_eta, wtk, metal_valence,nkpt,nband,Rx, Ry,
                   extrinsic,pjt1,pjt2,pjt3,cs1,cs2,cs3):
    import numpy as np
