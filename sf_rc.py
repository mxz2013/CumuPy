...
def calc_rc_Josh (ehf,wps1,wps2,gwfermi, lda_fermi, bdrange, bdgw_min, kptrange, FFTtsize, en,enmin, enmax,
                    eqp, Elda, scgw, ims, invar_den, invar_eta, wtk,nkpt,nband,
            Rx, Ry, extrinsic, core ,Eplasmon,pjt1,pjt2,pjt3,cs1,cs2,cs3):
    import numpy as np
    import pyfftw
    ...
    fftsize = FFTtsize
    norm = np.zeros((nkpt,nband))
    outname = "Norm_check_rc_Josh.dat"
    outfile = open(outname,'w')
    trange, dtfft,denfft, fften_min  = prep_FFT(invar_den, fftsize)
    for ik in kptrange:
        ikeff = ik + 1
        ...
        for ib in bdrange:
            ...
            w_list, gw_list = calc_FFT0(ehf_kb,gt_list, fftsize,dtfft, enmin, newen_rc, denfft, invar_eta)
            ...
            with open("spf_rc_Josh-k"+str("%02d"%(ikeff))+"-b"+str("%02d"%(ibeff))+"-ext"+str(extrinsic)+".dat", 'w') as f:
                writer = csv.writer(f, delimiter = '\t')
                writer.writerow(['# w-fermi','# spf_toc11',
                                 'spf_toc11_pjts',
                                 'spf_toc11_pjtp','spf_toc11_pjtd',
        ...
        with open("spf_rc_Josh-k"+str("%02d"%(ikeff))+".dat", 'w') as f:
            writer = csv.writer(f, delimiter = '\t')
            writer.writerow(['# w-fermi','# spf_toc11 sum on b','# pjts',
                             '# pjtp','# pjtd', 'sum b and projections'])
            writer.writerows(zip (interp_en-gwfermi, spf_sumb, spf_sumb_pjt1,
...
def calc_rc_Josh_old (gwfermi, lda_fermi, bdrange, bdgw_min, kptrange, FFTtsize, en,enmin, enmax,
